...
const EMPTY: SpotifyStatus = {
  ...
  trackId: null, title: null, artist: null, thumb: null,
}

// ─── Client Credentials token ─────────────────────────────────────────────────

let _ccToken:  string | null = null
let _ccExpiry: number        = 0
...
async function resolveTrackUri(artist: string, title: string): Promise<string | null> {
  try {
...
const wsl = {
  ...
  async status(): Promise<SpotifyStatus> {
    try {
      ...
      if (dash === -1) return EMPTY
      return { ...EMPTY, isPlaying: true, artist: win.slice(0, dash).trim(), title: win.slice(dash + 3).trim() }
    } catch { return EMPTY }
  },
}
...
const macos = {
  ...
  async seek(ms: number)    { await osa(`tell application "Spotify" to set player position to ${ms / 1000}`) },
  async volume(pct: number) { await osa(`tell application "Spotify" to set sound volume to ${Math.round(pct)}`) },
  async status(): Promise<SpotifyStatus> {
    try {
      ...
      return {
        ...
        title: title ?? null, artist: artist ?? null, thumb: thumb ?? null,
      }
    } catch { return EMPTY }
  },
}
...
const windows = {
  ...
  async seek(_ms: number)    { },
  async volume(_pct: number) { },
  async status(): Promise<SpotifyStatus> { return EMPTY },
}
...
export function registerSpotifyIpc() {
  ...
  ipcMain.handle('spotify:seek',   (_e, ms: number)                   => ctrl().seek(ms))

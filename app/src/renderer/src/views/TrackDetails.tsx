...
export function TrackDetails({ track, onClose, onCallAgent, onFilter }: TrackDetailsProps) {
  useEffect(() => {
        ...
        <div className="td-info">
          ...
          {track.moods.length > 0 && (
            <div className="td-tags td-tags--moods">
              {track.moods.map(m => (
                <span key={m} className="td-tag td-tag--mood" style={{ cursor: 'pointer' }} onClick={() => handleFilter('mood', m)}>
                  {m}
                </span>
              ))}
            </div>
          ...
          {track.contexts.length > 0 && (
            <div className="td-tags td-tags--contexts">
              {track.contexts.map(c => (
                <span key={c} className="td-tag td-tag--context" style={{ cursor: 'pointer' }} onClick={() => handleFilter('context', c)}>
                  {c}
                </span>
              ))}
            </div>
